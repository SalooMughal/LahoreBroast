...
class EditProfile : Fragment(), ProfileViewModel.ServerResponse {
    ...
    override fun onActivityCreated(savedInstanceState: Bundle?) {
        ...
        updateEdit.setOnClickListener {
            ...
            if (passwordProfile.text.isNotEmpty() && passwordProfile.text.length >= 8) {
            ...
            if (editAddress.text.toString().isEmpty()) {
                Utilities.showAlertDialog(activity!!, "Please Check your Address")
                return@setOnClickListener
            }
            ...
            if (editUserName.text.toString().isNotEmpty() && editEmail.text.toString()
                    .isNotEmpty() &&
                editNumber.text.toString().isNotEmpty() &&
                editNumber.text.length >= 11 && android.util.Patterns.EMAIL_ADDRESS.matcher(
                    editEmail.text
                ).matches() && editAddress.text.toString().isNotEmpty()
            ) {
